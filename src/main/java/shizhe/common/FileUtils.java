...
public class FileUtils extends org.apache.commons.io.FileUtils {
	...
	/**
	 * 移动单个文件
	 * @param oriPath 待复制的文件名
	 * @param cover 是否覆盖已经存在的文件
	 * @return 如果复制成功，则返回true，否则返回false
	 */
	public static boolean moveUploadFileCover(String oriPath, boolean cover) {
		String fileName = FileUtils.getFileName(oriPath);
		String originPath = Global.getUploadSysPath() + oriPath;
		String destPath = Global.getAbsoluteUploadDir() + fileName;
		if(!copyFileCover(originPath, destPath, cover)){
			return false;
		}
		if (!deleteFile(originPath)){
			return false;
		}
		return true;
	}
	 ...
	 * 删除上传的文件
	 *
	 * @param oriPath 被删除的文件名
	 * @return 如果删除成功，则返回true，否则返回false
	 */
	public static boolean deleteUploadFile(String oriPath) {
		String fileName = FileUtils.getFileName(oriPath);
		String originPath = Global.getUploadSysPath() + oriPath;
		return deleteFile(originPath);
	}
	...
	public static String getFileName(String fileName) {
		if ((fileName == null) || (fileName.lastIndexOf(File.separator) == -1) || (fileName.lastIndexOf(File.separator) == fileName.length() - 1)) {
			return null;
		}
		return StringUtils.lowerCase(fileName.substring(fileName.lastIndexOf(File.separator) + 1));
	}
